...
function HandleMinigame(rewards)
	...
	TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Not defterinde kasanın şifrelerini buldun. (Bir tur sola) ' .. (math.floor((lockNumbers[1] % 360) / 3.60)) .. '. (Sağa) ' .. (math.floor((lockNumbers[2] % 360) / 3.60)) .. '. (Bir tur sola) ' .. (math.floor((lockNumbers[3] % 360) / 3.60)) .. '. (Sağa) ' .. (math.floor((lockNumbers[4] % 360) / 3.60)) .. '.', length = 55000})
	Citizen.Wait(10)
	TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = '"LeftShift" tuşu ile hızlandırabilir, "Z" tuşu ile yavaşlatabilirsiniz. "G" tuşu ile kapatabilirsiniz.', length = 54990})
	--------------------------------------
